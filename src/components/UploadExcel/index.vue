...
// 5.出于 上传文件之前可能需要对文件校验的场景 提供beforeUpload 字段，可以使用者自由定义校验规则。
// 6.利用XLSX插件读取xlsx文件里的数据，拿到数据，对数据进行整合
// 7. 把整合的数据抛出去
// 8.这个组件给父组件提供了两个参数beforeUpload 上传前的校验onSuccess. 上传成功拿到数据之后可以通过这个回调把数据给父组件
